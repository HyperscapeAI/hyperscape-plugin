...
import type { Player, ClientNetwork } from "../types/core-types";
...
export class EmoteManager {
  ...
  async uploadEmotes() {
    ...
    if (!service || !world) {
      console.warn("[EmoteManager] Cannot upload emotes - service or world not available");
      return;
    }

    const network = world.network as ClientNetwork;
    if (!network?.upload) {
      console.warn("[EmoteManager] Cannot upload emotes - network.upload not available");
      return;
    }
  ...
  async playEmote(emoteName: string): Promise<void> {
    ...
    const world = service?.getWorld();
    if (!service || !world) {
      console.warn("[EmoteManager] Cannot play emote - service or world not available");
      return;
    }

    const agentPlayer = world.entities?.player as Player | undefined;
    if (!agentPlayer) {
      console.warn("[EmoteManager] Cannot play emote - player not available");
      return;
    }
