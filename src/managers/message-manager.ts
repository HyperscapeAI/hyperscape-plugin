...
export class MessageManager {
  ...
  async handleMessage(msg: ChatMessage): Promise<void> {
    ...
    if (!service || !world) {
      console.warn("[MessageManager] Service or world not available");
      return;
    }
    ...
    // Get room ID from player or use a default
    const roomId = world.entities?.player?.data?.id || crypto.randomUUID();
...
Generate a natural chat response that fits the conversation flow.
  ...
  async sendMessage(text: string): Promise<void> {
    ...
    const world = service?.getWorld();
    if (!service || !world) {
      console.warn("[MessageManager] Cannot send message - service or world not available");
      return;
    }

    const player = world.entities?.player;
    if (!player) {
      console.warn("[MessageManager] Cannot send message - player not available");
      return;
    }
  ...
  private getWorldContext(world: World): string {
    ...
    // Add position if player and node are available
    if (player?.node?.position) {
      const pos = player.node.position;
      context.push(`Agent position: (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)})`);
    }
