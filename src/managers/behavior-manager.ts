...
export class BehaviorManager {
  ...
  private async executeBehavior(): Promise<void> {
    ...
    const roomId = this.world?.entities?.player?.data?.id || crypto.randomUUID();
...
<thought>Brief explanation of your message</thought>
  ...
  private async handleChatAction(content: ResponseContent): Promise<void> {
    ...
    if (!messageManager) {
      console.warn("[BehaviorManager] Cannot send chat - message manager not available");
      return;
    }
  ...
  private async getRecentChatHistory(): Promise<string> {
    ...
    if (!messageManager || !world) return "No chat history available";
