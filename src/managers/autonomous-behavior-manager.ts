...
import { KNOWN_LOCATIONS } from "../goals/templates.js";
...
export class AutonomousBehaviorManager {
  ...
  /**
   * Get goal suggestions from the domain registry (if available)
   * Falls back to empty array if plugin-goals is not loaded
   */
  async getGoalSuggestions(): Promise<Array<{ id: string; name: string; description?: string; priority: number }>> {
    try {
      ...
      const registry = (this.runtime as unknown as { getService: (name: string) => unknown }).getService(DomainRegistryClass.serviceName) as {
        getSuggestions: (runtime: unknown) => Array<{ id: string; name: string; description?: string; priority: number }>;
      } | undefined;
      
      if (registry) {
        const suggestions = registry.getSuggestions(this.runtime);
      ...
      // plugin-goals not available
    }
    return [];
  }

  /**
   * Get recommended action for current goal from domain registry (if available)
   * Falls back to null if plugin-goals is not loaded
   */
  async getRecommendedAction(): Promise<{ action: string; params?: Record<string, unknown> } | null> {
    if (!this.currentGoal) return null;
    
    try {
      const goalsModule = await import('@elizaos/plugin-goals');
      const DomainRegistryClass = (goalsModule as Record<string, unknown>).DomainRegistry as { serviceName: string } | undefined;
      if (!DomainRegistryClass) return null;
      
      const registry = (this.runtime as unknown as { getService: (name: string) => unknown }).getService(DomainRegistryClass.serviceName) as {
        getRecommendedAction: (goal: unknown, runtime: unknown) => { action: string; params?: Record<string, unknown> } | null;
      } | undefined;
      
      if (registry) {
        // Create a mock GoalData for the domain evaluator
        const mockGoal = {
          id: 'current-goal',
          agentId: this.runtime.agentId,
          ownerType: 'agent',
          ownerId: this.runtime.agentId,
          name: this.currentGoal.description,
          goalType: 'objective',
          progress: this.currentGoal.progress,
          priority: 1,
          status: 'active',
          isCompleted: false,
          createdAt: new Date(this.currentGoal.startedAt),
          updatedAt: new Date(),
          metadata: {
            type: this.currentGoal.type,
            location: this.currentGoal.location,
            targetEntity: this.currentGoal.targetEntity,
            targetSkill: this.currentGoal.targetSkill,
            targetSkillLevel: this.currentGoal.targetSkillLevel,
          },
          criteria: this.currentGoal.targetSkill ? {
            type: 'counter',
            domain: 'hyperscape',
            counter: {
              path: `player.skills.${this.currentGoal.targetSkill}.level`,
              target: this.currentGoal.targetSkillLevel || this.currentGoal.target,
              initial: 0,
            },
          } : undefined,
        };
        
        return registry.getRecommendedAction(mockGoal, this.runtime);
      }
    } catch {
      // plugin-goals not available
    }
