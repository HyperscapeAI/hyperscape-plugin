...
export class MultiAgentManager extends EventEmitter {
  ...
  async addAgent(runtime: IAgentRuntime): Promise<AgentInstance> {
    ...
    // Move agent to spawn position if world and controls are available
    const world = service.getWorld();
    if (world) {
      // Try to find a controls system that has goto method
      const controlsSystem = world.systems?.find(
        (sys) => sys && typeof (sys as { goto?: (x: number, z: number) => void }).goto === 'function'
      ) as { goto: (x: number, z: number) => void } | undefined;
      controlsSystem?.goto(spawnX, spawnZ);
    }
  ...
  private updateAgents(): void {
    for (const agent of this.agents.values()) {
      ...
      // Update position if player and node are available
