...
import { THREE, type Player } from "../types/core-types";
...
export class PlaywrightManager {
  ...
  async takeScreenshot(
    ...
    if (!this.page) {
      throw new Error("PlaywrightManager page not initialized");
    }
  ...
  public async snapshotFacingDirection(
    ...
    const rotation = player.rotation as THREE.Quaternion | undefined;
    const playerData = {
      position: player.node.position.toArray() as [number, number, number],
      rotation: rotation?.toArray() as [number, number, number, number] || [0, 0, 0, 1],
    };
  ...
  public async snapshotEquirectangular(): Promise<string> {
    ...
    const playerData = {
      ...
      quaternion: player.node.quaternion?.toArray() || [0, 0, 0, 1],
    };
      ...
      async ({
        ...
        gltf.scene.traverse((obj: THREE.Object3D) => {
          ...
          mats.forEach((mat) => {
            ...
            if (!m.userData.materialId) {
              m.userData.materialId = window.crypto.randomUUID();
            }
            const id = m.userData.materialId;

            STRIP_SLOTS.forEach((slot) => {
              const tex = m[slot] as THREE.Texture;
    ...
    await this.page.evaluate(
      async ({
        ...
        players.forEach((player: Player) => {
          ...
          const rotation = player.node.quaternion || new THREE.Quaternion();
          const scale = player.node.scale as THREE.Vector3 || new THREE.Vector3(1, 1, 1);

          const matrix = new THREE.Matrix4();
          matrix.compose(position, rotation as THREE.Quaternion, scale);
