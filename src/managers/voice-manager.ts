...
export class VoiceManager {
  ...
  start() {
    ...
    if (!world || !world.livekit?.on) {
      console.error("[VoiceManager] World or LiveKit not available");
      return;
    }

    world.livekit.on("audio", async (rawData: unknown) => {
      const data = rawData as LiveKitAudioData;
  ...
  private async handleMessage(message: string, playerId: UUID) {
    ...
    const world = service?.getWorld();
    if (!service || !world) {
      logger.error("[VoiceManager] Service or world not available");
      return { text: "", actions: ["IGNORE"] };
    }

    const playerInfo = world.entities?.getPlayer?.(playerId);
    if (!playerInfo) {
      logger.warn(`[VoiceManager] Player info not found for ${playerId}`);
      return { text: "", actions: ["IGNORE"] };
    }

    const userName = playerInfo.data?.name || "Unknown";
    const name = userName;
    const _currentWorldId = service.currentWorldId || crypto.randomUUID() as UUID;
    ...
    const callback: HandlerCallback = async (
      ...
      if (responseMemory.content.text?.trim()) {
        ...
        const audioBuffer = await convertToAudioBuffer(responseStream as Buffer);
    ...
    // Emit voice message event using the custom hyperscape event system
    // Cast to satisfy TypeScript - this is a custom event type handled by hyperscapeEvents
  ...
  async playAudio(audioBuffer: Buffer) {
    ...
    const world = service?.getWorld();
    if (!service || !world) {
      logger.error("[VoiceManager] Service or world not available for audio playback");
      return;
    }
