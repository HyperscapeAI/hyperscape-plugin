...
export class ContentPackLoader {
  ...
  private loadVisualConfig(visuals: IVisualConfig): void {
    ...
    if (visuals.entityColors) {
      const colorDetector = world.colorDetector as { registerEntityColor?: (entityType: string, config: unknown) => void } | undefined;
      const registerFn = colorDetector?.registerEntityColor;
      if (registerFn) {
        Object.entries(visuals.entityColors).forEach(([entityType, config]) => {
          registerFn.call(colorDetector, entityType, config);
        });
      }
    }
    ...
    // Load assets
    if (visuals.assets) {
      const assetLoader = (world as unknown as { assetLoader?: { loadModel: (url: string) => void } }).assetLoader;
      if (assetLoader?.loadModel && visuals.assets.models) {
