...
export function createMockWorld(overrides: Partial<World> = {}): World {
  // Internal map for system lookup
  const systemsByName = new Map<string, unknown>();
  ...
  const mockWorld = {
    ...
    getSystem: vi.fn((systemKey: string) => {
      return (
        systemsByName.get(systemKey) ||
        (mockWorld as Record<string, unknown>)[systemKey] ||
        null
      );
    }),
    ...
    get isServer() {
      return (mockWorld.network as { isServer?: boolean }).isServer || false;
    },
    get isClient() {
      return (mockWorld.network as { isClient?: boolean }).isClient || true;
    },
