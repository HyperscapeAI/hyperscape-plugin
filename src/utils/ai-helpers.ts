...
export function composeContext(options: ComposeContextOptions): string {
  ...
  const replacements: Record<string, unknown> = {
    ...
    characterBio,
    currentLocation:
      (state?.values as Record<string, unknown>)?.["currentLocation"] || "Unknown Location",
    recentMessages:
      (state?.values as Record<string, unknown>)?.["recentMessages"] || "No recent messages",
    ...additionalContext,
  };
...
export async function generateMessageResponse(
  ...
  const { runtime, context, modelType = ModelType.TEXT_LARGE, stop = [] } = options;

  const response = await runtime.useModel(modelType as "TEXT_SMALL" | "TEXT_LARGE", {
    prompt: context,
    maxTokens: 1000,
    temperature: 0.8,
...
export async function generateDetailedResponse(
  ...
  const response = (await runtime.useModel(
    (options.modelType || ModelType.TEXT_LARGE) as "TEXT_SMALL" | "TEXT_LARGE",
    {
      prompt: context,
      maxTokens: 2000,
      temperature: 0.8,
