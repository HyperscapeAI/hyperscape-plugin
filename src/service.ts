...
// Local types - no @hyperscape/shared dependency, no PhysX required
import {
  THREE,
  type Chat,
  type ClientInput,
 ...
 * Note: We use a separate interface instead of extending Player to avoid type conflicts
 */
interface ModifiablePlayer {
...
- Manage multi-agent interactions in virtual environments
  ...
  async connect(config: {
    ...
    // Note: Uses lightweight mock physics (no PhysX WASM required)
    ...
    // Register controls system - ClientInput interface provides both human and agent control
    // Note: In mock world, we use a placeholder since ClientInput is an interface
    if (this.world.register) {
      this.world.register("controls", {} as unknown);
    }
  ...
  private subscribeToHyperscapeEvents(): void {
    if (!this.world?.off || !this.world?.on) return;
    ...
    this.world.on("disconnect", (data: unknown) => {
      ...
      // Emit world left event with proper payload
      this.runtime.emitEvent(ElizaEventType.WORLD_LEFT, {
        runtime: this.runtime,
        source: "hyperscape",
        world: {
          id: this._currentWorldId || this.runtime.agentId,
          name: "hyperscape",
  ...
  async disconnect(): Promise<void> {
    ...
    (
      this.runtime as {
        emitEvent: (type: ElizaEventType, data: unknown) => void;
      }
    ).emitEvent(ElizaEventType.WORLD_LEFT, {
      runtime: this.runtime,
  ...
  async loadUGCContent(
    ...
    this.runtime.emitEvent(ElizaEventType.WORLD_JOINED, {
      runtime: this.runtime,
      source: "hyperscape",
      world: {
        id: this._currentWorldId || this.runtime.agentId,
        name: contentBundle.name || contentId,
        agentId: this.runtime.agentId,
        metadata: {
          contentId,
  ...
  /**
   * Create a minimal world implementation with lightweight mock physics
   * Note: This uses simple JavaScript physics simulation, not PhysX WASM
   */
  private createWorld(config: MockWorldConfig): World {
    console.info("[MinimalWorld] Creating minimal world with mock physics (no PhysX)");
    ...
    const minimalWorld: Partial<World> = {
      ...
      // Mock physics system - lightweight JS simulation, no PhysX WASM required
      // This provides basic movement/gravity for AI agent testing without heavy dependencies
      physics: {
        enabled: true,
        gravity: { x: 0, y: -9.81, z: 0 },
        timeStep: 1 / 60,
        world: null, // Not using PhysX - this is mock physics
      ...
      entities: {
        ...
        players: new Map(),
        items: new Map(),
        add: ((data: unknown, _local?: boolean) => {
          const entityData = data as { id?: string; type?: string; position?: Vector3; [key: string]: unknown };
          console.log("[MinimalWorld] Entity added:", entityData.id || "unknown");
          // Handle both Entity objects and EntityData - check if it has required Entity properties
          const isEntity = entityData && typeof entityData === 'object' && 'id' in entityData && 'type' in entityData && 'data' in entityData;
          const entity: Entity = isEntity ? entityData as Entity : {
      ...
      init: async (initConfig?: Partial<MockWorldConfig>) => {
        ...
        minimalWorld.entities!.player = {
          ...
          walkToward: (targetPosition: Position, speed: number = 5) => {
            ...
            const currentPos = player?.position || { x: 0, y: 0, z: 0 };
            const controller = minimalWorld.physics?.controllers?.get(playerId);
          ...
          teleport: (options: TeleportOptions) => {
            ...
            if (options.position && minimalWorld.entities?.player) {
              const player = minimalWorld.entities.player;
              // Update player position
              if (player.position) {
                Object.assign(player.position, options.position);
              } else {
          ...
          modify: (data: EntityModificationData) => {
            ...
            if (minimalWorld.entities?.player?.data) {
