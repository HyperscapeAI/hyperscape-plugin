...
import { THREE, System, type World } from "../types/core-types";
...
export class EnvironmentSystem extends System {
  ...
  start() {
    ...
    if (this.world.settings?.on) {
      this.world.settings.on("change", (data: unknown) => this.onSettingsChange(data as Record<string, unknown>));
    }
  ...
  async updateModel() {
    ...
    const loader = this.world.loader;
    if (!loader?.get || !loader?.load) return;
    let glb = loader.get("model", url);
    if (!glb) {
      glb = await loader.load("model", url);
    }
    if (this.model) {
      const deactivate = (
  ...
  async updateSky() {
    ...
    let bgUUID;
    if (bgUrl) {
      const resolvedBgUrl = await resolveUrl(bgUrl as string, this.world);
      if (resolvedBgUrl) {
        bgUrl = resolvedBgUrl;
        bgUUID = await playwrightManager.registerTexture(resolvedBgUrl, "map");
      }
    }
    if (bgUUID) {
  ...
  lateUpdate(_delta: number) {
    if (!this.sky || !this.world.rig?.position) {
      return;
    }
