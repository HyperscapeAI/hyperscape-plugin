// Math utilities for movement calculations - local types, no @hyperscape/shared
import { THREE, type World, type Player, type Vector3, type Entity } from "../types/core-types";
...
export class PlayerMovementSystem extends EventEmitter {
  ...
  startMovement(playerId: string, target: Vector3, path?: Vector3[]): void {
    ...
    const finalPath = path ||
      (player.node?.position ? this.findPath(player.node.position, target) : null) || [target];
    ...
    if (this.world.network.send && player.node?.position) {
  ...
  private calculateSlideVelocity(player: MovablePlayer): Vector3 | null {
    const nodePosition = player.node?.position;
    if (!player.velocity || !nodePosition || !player.speed) {
      return null;
    }
  ...
  private sendNetworkUpdates(): void {
    ...
    for (const [playerId, _movement] of this.movingPlayers) {
      const player =
        this.world.entities.players?.get(playerId) ||
        (playerId === this.world.entities.player?.id
          ? this.world.entities.player
          : null);
      if (!player || !player.node?.position) continue;

      // Send via network - reuse pre-allocated payload object
      if (this.world.network.send) {
        this._networkUpdatePayload.playerId = playerId;
        this._networkUpdatePayload.position = player.node.position as THREE.Vector3;
